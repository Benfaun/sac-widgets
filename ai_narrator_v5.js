/**
 * SAC Custom Widget — AI Narrator v5
 * Multi-medida, multi-moneda, multi-dimension, comparacion de periodos,
 * prompt personalizable, historial de chat, re-analisis automatico
 */
...
(function () {
  ...
  class AINarrator extends HTMLElement {
    constructor() {
      ...
      this._narratorLanguage   = "es";
    }

    connectedCallback() { this._render(); }

    // ── Propiedades configurables desde Builder panel ─────────────────────────
    get customSystemPrompt() { return this._customSystemPrompt; }
    set customSystemPrompt(val) { this._customSystemPrompt = val || ""; }

    get narratorLanguage() { return this._narratorLanguage; }
    set narratorLanguage(val) { this._narratorLanguage = val || "es"; }

    // ── SAC Data Binding ──────────────────────────────────────────────────────
    onCustomWidgetAfterUpdate(changedProperties) {
      if ("customSystemPrompt" in changedProperties) {
        this._customSystemPrompt = changedProperties["customSystemPrompt"] || "";
      }
      if ("narratorLanguage" in changedProperties) {
        this._narratorLanguage = changedProperties["narratorLanguage"] || "es";
      }
    ...
    // ── Parsear DataBinding — todas las dims, medidas y monedas ──────────────
    _parseBinding(db) {
      try {
        ...
        // Filas completas con todas las dims y medidas
        const richRows = rows.map(row => ({
          dims: dimKeys.map((k, i) => ({
            label: dimLabels[i],
            value: row[k]?.label || row[k]?.id || "N/A"
          })),
          measures: measKeys.map((k, i) => ({
            label:     measLabels[i],
            raw:       row[k]?.raw,
        ...
        // Clave multi-dimensional para agrupamiento
        const groups = {};
        richRows.forEach(r => {
          const key = r.dims.map(d => d.value).join(" / ");
          const val = r.measures[0]?.raw || 0;
          groups[key] = (groups[key] || 0) + val;
        });

        // Detectar si hay dimension de fecha/periodo
        const periodDimIndex = this._detectPeriodDimension(richRows, dimLabels);
        ...
        return {
          ...
          filters: db.filters || {},
          metadata: {
            dimensions:    dimLabels.join(", ")  || "Dimension",
            measures:      measLabels.join(", ") || "Measure",
    ...
    // Detecta si alguna dimension parece fecha o periodo
    _detectPeriodDimension(richRows, dimLabels) {
      if (richRows.length === 0) return -1;
      for (let i = 0; i < dimLabels.length; i++) {
        const label = dimLabels[i].toLowerCase();
        if (/date|fecha|period|periodo|year|ano|month|mes|quarter|trimestre/.test(label)) return i;
        // Verificar si los valores parecen fechas
        const sampleVal = richRows[0]?.dims[i]?.value || "";
        if (/^\d{4}$/.test(sampleVal) || /Q[1-4]/i.test(sampleVal) || /^\d{4}\.\d{2}/.test(sampleVal)) return i;
      }
      return -1;
    }

    // ── Construir contexto de datos enriquecido para el prompt ────────────────
    _buildDataContext(chartData) {
      ...
      if (m.currencies) lines.push(`Monedas: ${m.currencies} (no comparar entre monedas distintas)`);

      // Si hay dimension de periodo, mostrar comparacion temporal
      if (m.periodDimIndex >= 0 && m.richRows) {
        const periods = [...new Set(m.richRows.map(r => r.dims[m.periodDimIndex]?.value))].sort();
        if (periods.length >= 2) {
          lines.push(`\nPeriodos detectados: ${periods.join(", ")}`);
          lines.push(`Periodo actual: ${periods[periods.length - 1]} | Periodo anterior: ${periods[periods.length - 2]}`);

          // Calcular deltas por grupo (dims sin el periodo)
          const byGroup = {};
          m.richRows.forEach(r => {
            const period   = r.dims[m.periodDimIndex]?.value;
            const groupKey = r.dims.filter((_, i) => i !== m.periodDimIndex).map(d => d.value).join(" / ") || "Total";
            if (!byGroup[groupKey]) byGroup[groupKey] = {};
            byGroup[groupKey][period] = r.measures;
          });

          lines.push(`\nComparacion de periodos:`);
          const curr = periods[periods.length - 1];
          const prev = periods[periods.length - 2];
          Object.entries(byGroup).slice(0, 20).forEach(([group, perData]) => {
            const currMeas = perData[curr] || [];
            const prevMeas = perData[prev] || [];
            const measStr = currMeas.map((cm, i) => {
              const pm = prevMeas[i];
              const delta = pm && pm.raw !== 0
                ? ` (${cm.raw >= pm.raw ? "+" : ""}${(((cm.raw - pm.raw) / Math.abs(pm.raw)) * 100).toFixed(1)}%)`
                : "";
              return `${cm.label}: ${cm.formatted || cm.raw}${cm.unit ? " " + cm.unit : ""}${delta}`;
            }).join(", ");
            lines.push(`  ${group}: ${measStr}`);
          });
          return lines.join("\n");
        }
      }

      // Sin periodo — mostrar todas las filas con todas las medidas
    ...
    _getDemoData() {
      return {
        ...
        filters: {},
        metadata: {
          dimensions: "Organization", measures: "Revenue",
          currencies: "USD", totalRows: 3, periodDimIndex: -1,
          richRows: [
    ...
    _updateDataSummary() {
      ...
      if (m.periodDimIndex >= 0) txt += " \u2022 \u23F1 Periodos detectados";
    ...
    async _sendMessage(question, mode) {
      ...
      const lang      = this._narratorLanguage === "en" ? "English" : "español";

      if (mode === "chat" && question) this._addMessage("user", question);
      ...
      const currencyNote = chartData.metadata?.currencies
        ? `Los datos pueden estar en distintas monedas (${chartData.metadata.currencies}). No comparar valores de monedas diferentes sin aclararlo.`
        : "";

      const hasPeriods = chartData.metadata?.periodDimIndex >= 0;

      // System prompt: usa el personalizado si existe, sino el default
      const defaultSystem = `Eres un analista de negocio experto en SAP Analytics Cloud.
Dimensiones disponibles: ${chartData.metadata?.dimensions}.
Medidas disponibles: ${chartData.metadata?.measures}.
${currencyNote}
${hasPeriods ? "Los datos incluyen multiples periodos. Enfocate en tendencias y comparaciones temporales." : ""}
Responde siempre en ${lang}, en tono ejecutivo y conciso.${usingDemo ? " AVISO: usando datos demo." : ""}`;

      const systemPrompt = this._customSystemPrompt
        ? `${this._customSystemPrompt}\nIdioma de respuesta: ${lang}.${usingDemo ? " AVISO: usando datos demo." : ""}`
        : defaultSystem;

      // Historial reciente para modo chat
      const historyContext = mode === "chat" && this._history.length > 0
        ? `\nHistorial reciente:\n${this._history.slice(-6).map(h =>
            `${h.role === "user" ? "Usuario" : "Asistente"}: ${h.content.slice(0, 200)}`
          ).join("\n")}\n`
        : "";

      const autoPrompt = hasPeriods
        ? `${dataContext}\n\nGenera una narrativa ejecutiva de 4-5 oraciones con enfoque en la evolucion temporal. Incluye: comparacion entre periodos con porcentajes de cambio, los valores mas destacados del periodo actual, tendencias identificadas, y una recomendacion accionable basada en la evolucion.`
        : `${dataContext}\n\nGenera una narrativa ejecutiva de 4-5 oraciones. Incluye: tendencias principales, top y bottom performers por dimension, comparacion entre medidas clave${chartData.metadata?.currencies ? ", considerando las distintas monedas" : ""}, y una recomendacion accionable.`;

      const userPrompt = mode === "auto"    ? autoPrompt
        : mode === "suggest" ? `${dataContext}\n\nEres un consultor de SAP Analytics Cloud. Sugiere 3 visualizaciones especificas para este modelo.\n\nPara cada una indica:\n- Tipo de grafico SAC (Bar, Line, Waterfall, Pie/Donut, Bullet, Heat Map, Scatter, etc.)\n- Titulo sugerido\n- Dimension(es) para el eje\n- Medida(s) a mostrar\n- Filtros recomendados\n- Por que aporta valor al negocio\n\nUsa los nombres exactos de dimensiones y medidas del modelo. Formato: lista numerada, concisa.`
        : `${dataContext}${historyContext}\n\nPregunta: ${question}\n\nResponde de forma concisa y precisa con valores especificos del modelo.`;

      try {
        const resp = await fetch(`${ENDPOINT}/analyze`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chart_data: chartData, custom_prompt: userPrompt, system_prompt: systemPrompt })
        });
        ...
        this._history.push({ role: "user",      content: mode === "auto" ? "[auto]" : question || mode });
        ...
        this.dispatchEvent(new CustomEvent("onNarrationReady", {
          detail: { narration: result.narration }, bubbles: true
        }));
      } catch(err) {
        this._removeMessage(loadingId);
    ...
    _setButtons(disabled) {
      ["btn-analyze", "btn-suggest", "btn-send"].forEach(id => {
        const el = this._shadow.getElementById(id);
